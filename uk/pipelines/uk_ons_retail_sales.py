"""
ONS Retail Sales Index pipeline
...
def run():
    print(f"[{datetime.utcnow().isoformat()}] Running ONS Retail Sales pipeline...")
    csv_url = get_csv_url()
    print(f"  CSV: {csv_url}")
    ...
    with duckdb.connect(DB_PATH) as db:
        ...
        db.execute("DELETE FROM uk_ons_retail_sales WHERE source_id = ?", [source_id])
        ...
        for row in rows:
            ...
            db.execute(
                """
                INSERT INTO uk_ons_retail_sales
                (period_label, sector_code, sector_label, price_type,
        ...
        db.execute(
            ...
            WHERE id = 'uk_ons_retail_sales'
        """,
            [datetime.utcnow().isoformat(), loaded],
        )

    print(f"  ✓ Loaded {loaded:,} rows into uk_ons_retail_sales")
